...
export function createInitialJobs(
	...
	const jobs: readonly Job[] = messageSegmentComputers.map(
		(messageSegmentComputer: MessageSegmentComputer): Job => {
			const visitor = new InitialJobsMessageSegmentComputerVisitor();
			const job: Job = messageSegmentComputer.acceptVisitor(visitor);
			return job;
		}
	);
	return jobs;
